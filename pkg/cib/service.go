...
func (s *service) initSrc() error {
	...
	// Marshal
	def, err := state.Marshal(s.ctx, s.GetMarshalOpts()...)
	if err != nil {
		return err
	}
	// Solve
	res, err := s.client.Solve(s.ctx, client.SolveRequest{
		Definition: def.ToPB(),
	})
	if err != nil {
		return err
	}
	s.src, err = res.SingleRef()
	return err
}
...
func (s *service) initExcludes() error {
	...
	if err != nil {
		return errors.Wrapf(err, "failed to resolve excludes")
	}
	ref, err := res.SingleRef()
	if err != nil {
		return err
